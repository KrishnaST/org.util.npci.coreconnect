...
import org.util.npci.coreconnect.PropertyName;

//@formatter:off
public class CoreDatabaseServiceImpl implements CoreDatabaseService {
	
	private final CoreConfig config;
	private final String txTableName;
	private final boolean encrypted;
	
	public CoreDatabaseServiceImpl(final CoreConfig config) {
		this.config = config;
		this.txTableName = config.txTableName;
		this.encrypted = config.getBooleanSupressException(PropertyName.PAN_ENCRYPTED);
	}
	...
	public final long registerTransaction(final ISO8583Message message, final String type, final Logger logger) {
		...
		try(final Connection con = config.dataSource.getConnection();
			final PreparedStatement ps = con.prepareStatement("INSERT INTO "+txTableName+" (" + 
					...
					"VALUES (CONVERT(VARCHAR(64), HASHBYTES('SHA2_256', ?), 2) , ?, ?, ENCRYPTBYKEY(key_guid('sk_card'), ?, 1, ?), ?, " + 
					"?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, " + 
			...
			if(message.get(2) != null) {
				ps.setBytes(4, message.get(2).getBytes());
				ps.setBytes(5, getLast4Chars(message.get(2)).getBytes());
			}
			else {
	...
	public final ISO8583Message getTransaction(final long id, final Logger logger) {
		if(id == 0|| Strings.isNullOrEmpty(txTableName)) return null;
		try(final Connection con = config.dataSource.getConnection();
			final PreparedStatement ps = con.prepareStatement("SELECT CONVERT(VARCHAR, DECRYPTBYKEY(E002, 1, SUBSTRING(F002, (DATALENGTH(F002)-4), 4))) AS PAN, * FROM "+txTableName+" WHERE TXID = "+id);
	...
	public final ISO8583Message getTransaction(final String key, final Logger logger) {
		if(key == null || Strings.isNullOrEmpty(txTableName)) return null;
		try(final Connection con = config.dataSource.getConnection();
			final PreparedStatement ps = con.prepareStatement("SELECT CONVERT(VARCHAR, DECRYPTBYKEY(E002, 1, SUBSTRING(F002, (DATALENGTH(F002)-4), 4))) AS PAN, * FROM "+txTableName+" WHERE MKEY = CONVERT(VARCHAR(64), HASHBYTES('SHA2_256', '"+key+"'), 2)");
			final ResultSet rs = ps.executeQuery()) {
			if(rs.next()) {
				final ISO8583Message transaction = new ISO8583Message();
				transaction.put(0, rs.getString("F000"));
	...
	public final boolean registerResponse(final long id, final ISO8583Message response, final Logger logger) {
		if(id == 0|| Strings.isNullOrEmpty(txTableName)) return false;
		try(final Connection con = config.dataSource.getConnection();
	...
	public final boolean registerResponse(final String key, final ISO8583Message response, final Logger logger) {
		if(key == null|| Strings.isNullOrEmpty(txTableName)) return false;
		try(final Connection con = config.dataSource.getConnection();
			final PreparedStatement ps = con.prepareStatement("UPDATE "+txTableName+" SET R039 = ?, R038 = ?, RXTIME = GETDATE() WHERE MKEY = CONVERT(VARCHAR(64), HASHBYTES('SHA2_256', ?), 2)")){
			ps.setString(1, response.get(39));
			ps.setString(2, response.get(38));
			ps.setString(3, key);
			return ps.executeUpdate() > 0;
	...
	public final boolean isDuplicateTransaction(final String key, final Logger logger) {
		if(key == null || Strings.isNullOrEmpty(txTableName)) return false;
		try(final Connection con = config.dataSource.getConnection();
			final PreparedStatement ps = con.prepareStatement("SELECT CASE WHEN EXISTS (SELECT TXID FROM "+txTableName+" WITH (NOLOCK) WHERE MKEY = CONVERT(VARCHAR(64), HASHBYTES('SHA2_256', '"+key+"'), 2)) THEN CAST (1 AS BIT)ELSE CAST (0 AS BIT) END AS EXIST");
			final ResultSet rs = ps.executeQuery()){
