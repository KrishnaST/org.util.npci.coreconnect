...
public abstract class CoreDatabaseService {
	...
	public CoreDatabaseService(final CoreConfig config) {
		...
		this.encrypted = config.getBooleanSupressException(org.util.npci.coreconnect.CorePropertyName.PAN_ENCRYPTED);
		this.isdisabled = config.getBooleanSupressException(org.util.npci.coreconnect.CorePropertyName.IS_CORE_DATABASE_SERVICE_DISABLED);
	}
	...
	public final ISO8583Message getTransaction(final String key, final Logger logger) {
		if(isdisabled || key == null || Strings.isNullOrEmpty(txTableName)) return null;
		try(final Connection con = config.dataSource.getConnection();
